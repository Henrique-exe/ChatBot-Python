...
while True:
    ...
    if contexto_pergunta and entrada_lower in ["s", "sim", "n", "nao", "não"]:
        if entrada_lower in ["s", "sim"]:
            ...
            elif contexto_pergunta == "cancelamento":
                bot_falar(
                    "Cancelamento solicitado! Nossa equipe irá processar em breve. 🗑️")
                contexto_pergunta = None
                continue

            elif contexto_pergunta == "troca":
                bot_falar(
                    "Troca solicitada! Você receberá as instruções de devolução por e-mail em breve. 📦")
                contexto_pergunta = None
                continue

            elif contexto_pergunta == "rastreamento":
                bot_falar(
                    "Verifique o código de rastreio enviado para o seu e-mail cadastrado. 🔍")
                contexto_pergunta = None
                continue

            elif contexto_pergunta == "retirada_local":
                bot_falar(
                    "Nossa loja fica na Rua da FashionFlow, 123 - Centro. Horário: seg a sex das 9h às 18h. 🏬")
                contexto_pergunta = None
                continue

            elif contexto_pergunta == "produto_indisponivel":
                bot_falar(
                    "Anotado! Você será notificado por e-mail assim que o produto voltar ao estoque. 🔔")
                contexto_pergunta = None
                continue

            elif contexto_pergunta == "tamanho":
                bot_falar(
                    "Acesse nossa tabela de medidas em: fashionflow.com.br/tamanhos 📏")
                contexto_pergunta = None
                continue

            elif contexto_pergunta == "atendimento_humano":
                bot_falar(
                    "Conectando com um atendente humano... Por favor, aguarde. 👤")
                contexto_pergunta = None
                continue

            elif contexto_pergunta in ["reclamacoes", "elogios", "agradecimento"]:
                bot_falar(
                    "Obrigado pelo seu feedback! Ele é muito importante pra gente. 💙")
                contexto_pergunta = None
                continue
